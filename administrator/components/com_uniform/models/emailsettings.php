...
class JSNUniformModelEmailSettings extends JModelAdmin
{
	...
	public function getItem($pk = null)
	{
		...
		$action = !empty($_get['action']) ? $_get['action'] : $pk;
		$control = !empty($_get['control']) ? $_get['control'] : $_get['view'];
	...
	public function saveForm($data)
	{
		...
		{
			$data['template_attach'] = !empty($_POST['file_attach'])?json_encode($_POST['file_attach']):"";
		}
		else
		{
			$data['template_attach'] = !empty($_POST['file_attach_submiter'])?json_encode($_POST['file_attach_submiter']):"";
		}
