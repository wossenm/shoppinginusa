...
class JSNFormGenerateHelper
{
	...
	public static function fieldWebsite($data, $dataSumbission)
	{
		...
		$html = "<div class=\"control-group {$customClass} {$identify} {$hideField}\" data-id=\"{$data->id}\"><label for=\"{$identify}\" class=\"control-label\">" . htmlentities(JText::_($data->label), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label><div class=\"controls\"><input class=\"website {$requiredWebsite} {$sizeInput}\" id=\"{$data->id}\" name=\"{$data->id}\" type=\"text\" value=\"{$defaultValue}\" placeholder=\"" . htmlentities($placeholder, ENT_QUOTES, "UTF-8") . "\" /></div></div>";
		return $html;
	}
	...
	public static function fieldDate($data, $dataSumbission)
	{
		...
		$html = "<div class=\"control-group {$customClass} {$identify} {$hideField}\" data-id=\"{$data->id}\">
					<label for=\"{$identify}\" class=\"control-label\">" . htmlentities(JText::_($data->label), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label>
	...
	public static function fieldCurrency($data, $dataSumbission)
	{
		...
		$inputContent = "";
		$paymentActive = $data->options->paymentMoneyValue == "Yes" ? 'payment-active' : '';
		...
		$html = "<div class=\"control-group {$customClass} {$identify} {$hideField}\" data-id=\"{$data->id}\"><label for=\"{$identify}\" class=\"control-label\">" . htmlentities(JText::_($data->label), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label><div class=\"controls {$requiredBlank} currency-control clearfix\"><div class=\"clearfix\">{$inputContent}</div></div></div>";
		return $html;
	}
	...
	public static function fieldPhone($data, $dataSumbission)
	{
		...
		$html = "<div class=\"control-group {$customClass} {$identify} {$hideField}\" data-id=\"{$data->id}\"><label for=\"{$identify}\" class=\"control-label\">" . htmlentities(JText::_($data->label), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label><div class=\"controls {$requiredBlank}\">{$inputContent}</div></div>";
		return $html;
	}
	...
	public static function fieldName($data, $dataSumbission)
	{
		...
		$html = "<div class=\"control-group {$customClass} {$identify} {$hideField}\" data-id=\"{$data->id}\"><label for=\"{$identify}\" class=\"control-label\">" . htmlentities(JText::_($data->label), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label><div id=\"{$data->id}\" class=\"controls {$requiredBlank}\">";
		$valueFirstName = '';
		$valueLastName = '';
		$valueMiddle = '';
		...
		if ($data->options->autoInsertName == 1)
		{
	...
	public static function fieldFileUpload($data, $dataSumbission)
	{
		...
		$required = !empty($data->options->required) ? '<span class="required">*</span>' : '';
		$instruction = !empty($data->options->instruction) ? " <i original-title=\"" . htmlentities(JText::_($data->options->instruction), ENT_QUOTES, "UTF-8") . "\" class=\"icon-question-sign\"></i>" : '';
		$html = "<div class=\"control-group {$customClass} {$identify} {$hideField}\" data-id=\"{$data->id}\"><label for=\"{$identify}\" class=\"control-label\">" . htmlentities(JText::_($data->label), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label><div class=\"controls {$requiredBlank}\"><input id=\"{$data->id}\" class=\"input-file\" name=\"{$data->id}[]\" {$multiple} type=\"file\" /></div></div>";
		return $html;
	}
	...
	public static function fieldEmail($data, $dataSumbission)
	{
		...
		if($data->options->autoInsertEmail == 1)
		{
			$user = JFactory::getUser();
			if (!$user->guest)
			{
	...
	public static function fieldDropdown($data, $dataSumbission, $paymentType)
	{
		...
		$paymentActive = $data->options->paymentMoneyValue == "Yes" ? 'payment-active' : '';
		$html = "<div class=\"control-group {$customClass} {$identify} {$hideField}\" data-settings=\"" . htmlentities($dataSettings, ENT_QUOTES, "UTF-8") . "\" data-id=\"{$data->id}\"><label for=\"{$identify}\" class=\"control-label\">" . htmlentities(JText::_($data->label), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label><div class=\"controls {$requiredBlank}\"><select id=\"{$data->id}\" class=\"dropdown {$sizeInput} {$randomDropdown} {$paymentActive}\" name=\"{$data->id}\">";
		...
		if (isset($data->options->items) && is_array($data->options->items))
		{
			foreach ($data->options->items as $index => $option)
			{
				...
				else
				{
					...
					$html .= "<option class=\"jsn-column-item\" {$selected} {$selectDefault} attr-price=\"{$price}\" attr-qty=\"{$qty}\" value=\"" . htmlentities($value, ENT_QUOTES, "UTF-8") . "\">" . htmlentities($label, ENT_QUOTES, "UTF-8") . "</option>";
				}
			}
		}
	...
	public static function fieldList($data, $dataSumbission, $paymentType)
	{
		...
		if (isset($data->options->items) && is_array($data->options->items))
		{
			foreach ($data->options->items as $option)
			{
				...
				$html .= "<option class=\"jsn-column-item\" {$selected} attr-price=\"{$price}\" attr-qty=\"{$qty}\" value=\"" . $value . "\">" . htmlentities($label, ENT_QUOTES, "UTF-8") . "</option>";
			}
		}
		$html .= "</select></div></div>";
		return $html;
	...
	public static function fieldCountry($data, $dataSumbission)
	{
		...
		$defaultValue = !empty($dataSumbission[$data->id]) ? $dataSumbission[$data->id] : "";
		$sizeInput = !empty($data->options->size) ? $data->options->size : '';
		$html = "<div class=\"control-group {$customClass} {$identify} {$hideField} \" data-id=\"{$data->id}\"><label for=\"{$identify}\" class=\"control-label\">" . htmlentities(JText::_($data->label), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label><div class=\"controls {$requiredBlank}\"><select id=\"{$data->id}\" class=\"{$sizeInput}\" name=\"{$data->id}\">";
		if (isset($data->options->items) && is_array($data->options->items))
		{
			foreach ($data->options->items as $option)
			{
	...
	public static function fieldChoices($data, $dataSumbission, $paymentType)
	{
		...
		if (isset($data->options->items) && is_array($data->options->items))
		{
			foreach ($data->options->items as $i => $option)
			{
				...
				$html .= "<div class=\"jsn-column-item\"><label class=\"radio\"><input {$checked} name=\"{$data->id}\" attr-price=\"{$price}\" attr-qty=\"{$qty}\" value=\"" . strip_tags($value) . "\" type=\"radio\" />" . $label . "</label></div>";
			}
		}
		$textOthers = !empty($data->options->labelOthers) ? $data->options->labelOthers : "Others";
		if (!empty($data->options->allowOther))
	...
	public static function fieldCheckboxes($data, $dataSumbission, $paymentType)
	{
		...
		$paymentActive = $data->options->paymentMoneyValue == "Yes" ? 'payment-active' : '';
		$html = "<div class=\"control-group {$customClass} {$identify} {$hideField} \" data-settings=\"" . htmlentities($dataSettings, ENT_QUOTES, "UTF-8") . "\" data-id=\"{$data->id}\"><label for=\"{$identify}\" class=\"control-label\">" . htmlentities(JText::_($data->label), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label><div class=\"controls\"><div id=\"{$data->id}\" class=\"checkboxes jsn-columns-container {$data->options->layout} {$randomCheckbox} {$requiredCheckbox} {$paymentActive}\">";
		$defaultValue = isset($dataSumbission[$data->id]) ? $dataSumbission[$data->id] : "";
		if (isset($data->options->items) && is_array($data->options->items))
		{
			foreach ($data->options->items as $i => $option)
			{
				...
				$html .= "<div class=\"jsn-column-item\"><label class=\"checkbox\"><input {$checked} name=\"{$data->id}[]\" attr-price=\"{$price}\" attr-qty=\"{$qty}\" value=\"" . strip_tags($value) . "\" type=\"checkbox\" />" . $label . "</label></div>";
			}
		}
		$textOthers = !empty($data->options->labelOthers) ? $data->options->labelOthers : "Others";
		if (!empty($data->options->allowOther))
	...
	public static function fieldAddress($data, $dataSumbission)
	{
		...
		$html = "<div class=\"control-group {$customClass} jsn-group-field {$identify} {$hideField} \" data-id=\"{$data->id}\"><label for=\"{$identify}\" class=\"control-label\">" . htmlentities(JText::_($data->label), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label><div id=\"{$data->id}\" class=\"controls {$requiredBlank}\">";
	...
	public static function fieldLikert($data, $dataSumbission)
	{
		...
		$html = "<div class=\"control-group {$customClass} {$identify} {$hideField} \" data-settings=\"" . htmlentities($dataSettings, ENT_QUOTES, "UTF-8") . "\" data-id=\"{$data->id}\"><label for=\"{$identify}\" class=\"control-label\">" . htmlentities(JText::_($data->label), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label><div class=\"controls\">";
	...
	public static function fieldRecepientEmail($data, $dataSumbission)
	{
		...
		$sizeInput = !empty($data->options->size) ? $data->options->size : '';
		$multiple = !empty($data->options->multiple) ? "multiple" : "size='4'";
		$html = "<div class=\"control-group {$customClass} {$identify} {$hideField} \" data-id=\"{$data->id}\"><label for=\"{$identify}\" class=\"control-label\">" . htmlentities(JText::_($data->label), ENT_QUOTES, "UTF-8") . "{$required}{$instruction}</label><div class=\"controls {$requiredBlank}\"><select {$multiple} id=\"{$data->id}\" class=\"list {$sizeInput} {$randomList} \" name=\"{$data->id}[]\">";
		if (isset($data->options->items) && is_array($data->options->items))
		{
			foreach ($data->options->items as $option)
			{
				...
				$html .= "<option class=\"jsn-column-item\" {$selected} attr-email=\"{$email}\" value=\"" . $value . "\">" . htmlentities($label, ENT_QUOTES, "UTF-8") . "</option>";
			}
		}
		$html .= "</select></div></div>";
		return $html;
	...
	public static function fieldIdentificationCode($data, $dataSumbission)
	{
		...
		$html = "<div class=\"control-group {$customClass} {$identify} {$hideField}\" data-id=\"{$data->id}\"><label for=\"{$identify}\" class=\"control-label\">" . htmlentities(JText::_($data->label), ENT_QUOTES, "UTF-8") . "{$instruction}</label><div class=\"controls \"><span>$value</span><input type=\"hidden\" name=\"identification-code[{$data->id}]\" value=\"{$value}\"></div></div>";
		return $html;
	}
}