...
class JSNUniformHelper
{
	...
	public static function generateHTMLPages($formId, $formName, $formType = "", $topContent = "", $bottomContent = "", $showTitle = false, $showDes = false)
	{
		...
		if (strtolower($edition) != "free")
		{	
			if ((string) $items->form_payment_type != '')
			{	
								...
								<div class="controls">
									<div class="mc-subcriber">
										<div class="mc-subcriber-box">
											<h3><label class="checkbox"><input type="checkbox" name="mailchimp_subcriber" ' . $checked . '><span class="mc-subcriber-text">' . $mailchimpMsg . '</span></label></h3>
										</div>
									</div>
								</div>
							</div>';
