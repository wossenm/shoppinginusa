...
(function (window, document) {
	...
	MediumEditor.prototype = {
		defaults: {
			...
			extensions: {},
			activeButtonClass: 'es-text-editor-button-active',
			firstButtonClass: 'es-text-editor-button-first',
			lastButtonClass: 'es-text-editor-button-last'
		},
		...
		init: function (elements, options) {
			...
			while (this.options.elementsContainer.querySelector('#es-text-editor-toolbar-' + uniqueId)) {
				uniqueId = uniqueId + 1;
			}
		...
		bindClick: function(i) {
			...
			this.on(this.elements[i], 'click', function(){
				if (!self.options.disablePlaceholders) {
					// Remove placeholder
					this.classList.remove('es-text-editor-placeholder');
				}
		...
		activatePlaceholder:  function (el) {
			...
			if (!(el.querySelector('img')) &&
				...
				el.classList.add('es-text-editor-placeholder');
			}
		},
		placeholderWrapper: function (el, e) {
			el.classList.remove('es-text-editor-placeholder');
			if (e.type !== 'keypress') {
		...
		buttonTemplate: function (btnType) {
			var buttonLabels = this.getButtonLabels(this.options.buttonLabels),
				buttonTemplates = {
					'bold': '<button class="es-text-editor-action es-text-editor-action-bold" data-action="bold" data-element="b">' + buttonLabels.bold + '</button>',
					'italic': '<button class="es-text-editor-action es-text-editor-action-italic" data-action="italic" data-element="i">' + buttonLabels.italic + '</button>',
					'underline': '<button class="es-text-editor-action es-text-editor-action-underline" data-action="underline" data-element="u">' + buttonLabels.underline + '</button>',
					'strikethrough': '<button class="es-text-editor-action es-text-editor-action-strikethrough" data-action="strikethrough" data-element="strike">' + buttonLabels.strikethrough +'</button>',
					'superscript': '<button class="es-text-editor-action es-text-editor-action-superscript" data-action="superscript" data-element="sup">' + buttonLabels.superscript + '</button>',
					'subscript': '<button class="es-text-editor-action es-text-editor-action-subscript" data-action="subscript" data-element="sub">' + buttonLabels.subscript + '</button>',
					'anchor': '<button class="es-text-editor-action es-text-editor-action-anchor" data-action="anchor" data-element="a">' + buttonLabels.anchor + '</button>',
					'image': '<button class="es-text-editor-action es-text-editor-action-image" data-action="image" data-element="img">' + buttonLabels.image + '</button>',
					'header1': '<button class="es-text-editor-action es-text-editor-action-header1" data-action="append-' + this.options.firstHeader + '" data-element="' + this.options.firstHeader + '">' + buttonLabels.header1 + '</button>',
					'header2': '<button class="es-text-editor-action es-text-editor-action-header2" data-action="append-' + this.options.secondHeader + '" data-element="' + this.options.secondHeader + '">' + buttonLabels.header2 + '</button>',
					'quote': '<button class="es-text-editor-action es-text-editor-action-quote" data-action="append-blockquote" data-element="blockquote">' + buttonLabels.quote + '</button>',
					'orderedlist': '<button class="es-text-editor-action es-text-editor-action-orderedlist" data-action="insertorderedlist" data-element="ol">' + buttonLabels.orderedlist + '</button>',
					'unorderedlist': '<button class="es-text-editor-action es-text-editor-action-unorderedlist" data-action="insertunorderedlist" data-element="ul">' + buttonLabels.unorderedlist + '</button>',
					'pre': '<button class="es-text-editor-action es-text-editor-action-pre" data-action="append-pre" data-element="pre">' + buttonLabels.pre + '</button>',
					'indent': '<button class="es-text-editor-action es-text-editor-action-indent" data-action="indent" data-element="ul">' + buttonLabels.indent + '</button>',
					'outdent': '<button class="es-text-editor-action es-text-editor-action-outdent" data-action="outdent" data-element="ul">' + buttonLabels.outdent + '</button>',
					'justifyCenter': '<button class="es-text-editor-action es-text-editor-action-justifyCenter" data-action="justifyCenter" data-element="">' + buttonLabels.justifyCenter + '</button>',
					'justifyFull': '<button class="es-text-editor-action es-text-editor-action-justifyFull" data-action="justifyFull" data-element="">' + buttonLabels.justifyFull + '</button>',
					'justifyLeft': '<button class="es-text-editor-action es-text-editor-action-justifyLeft" data-action="justifyLeft" data-element="">' + buttonLabels.justifyLeft + '</button>',
					'justifyRight': '<button class="es-text-editor-action es-text-editor-action-justifyRight" data-action="justifyRight" data-element="">' + buttonLabels.justifyRight + '</button>'
				};
		...
		initToolbar: function () {
			...
			this.toolbarActions = this.toolbar.querySelector('.es-text-editor-toolbar-actions');
			this.anchorPreview = this.createAnchorPreview();

			if (!this.options.disableAnchorForm) {
				this.anchorForm = this.toolbar.querySelector('.es-text-editor-toolbar-form-anchor');
				this.anchorInput = this.anchorForm.querySelector('input.es-text-editor-toolbar-anchor-input');
				this.anchorTarget = this.anchorForm.querySelector('input.es-text-editor-toolbar-anchor-target');
				this.anchorButton = this.anchorForm.querySelector('input.es-text-editor-toolbar-anchor-button');
			}
			return this;
		},
		...
		createToolbar: function () {
			...
			toolbar.id = 'es-text-editor-toolbar-' + this.id;
			toolbar.className = 'es-text-editor-toolbar';
		...
		toolbarButtons: function () {
			...
			ul.id = 'es-text-editor-toolbar-actions' + this.id;
			ul.className = 'es-text-editor-toolbar-actions clearfix';
		...
		toolbarFormAnchor: function () {
			...
			close.className = 'es-text-editor-toobar-anchor-close';
			close.innerHTML = '&times;';

			save.setAttribute('href', '#');
			save.className = 'es-text-editor-toobar-anchor-save';
			save.innerHTML = '&#10003;';

			input.setAttribute('type', 'text');
			input.className = 'es-text-editor-toolbar-anchor-input';
			input.setAttribute('placeholder', this.options.anchorInputPlaceholder);


			target.setAttribute('type', 'checkbox');
			target.className = 'es-text-editor-toolbar-anchor-target';
			target_label.innerHTML = "Open in New Window?";
			target_label.insertBefore(target, target_label.firstChild);

			button.setAttribute('type', 'checkbox');
			button.className = 'es-text-editor-toolbar-anchor-button';
			button_label.innerHTML = "Button";
			button_label.insertBefore(button, button_label.firstChild);


			anchor.className = 'es-text-editor-toolbar-form-anchor';
			anchor.id = 'es-text-editor-toolbar-form-anchor-' + this.id;
		...
		setToolbarPosition: function () {
			...
			this.toolbar.classList.add('es-text-editor-toolbar-active');
		...
		hideToolbarActions: function () {
			this.keepToolbarAlive = false;
			if (this.toolbar !== undefined && this.toolbar.classList.contains('es-text-editor-toolbar-active')) {
				this.toolbar.classList.remove('es-text-editor-toolbar-active');
		...
		showToolbarActions: function () {
			...
			clearTimeout(timer);
			timer = setTimeout(function () {
				if (self.toolbar && !self.toolbar.classList.contains('es-text-editor-toolbar-active')) {
					self.toolbar.classList.add('es-text-editor-toolbar-active');
				}
			}, 100);
		},
		...
		bindAnchorForm: function () {
			...
			var linkCancel = this.anchorForm.querySelector('a.es-text-editor-toobar-anchor-close'),
				linkSave = this.anchorForm.querySelector('a.es-text-editor-toobar-anchor-save'),
				self = this;
		...
		hideAnchorPreview: function () {
			this.anchorPreview.classList.remove('es-text-editor-anchor-preview-active');
		},

		// TODO: break method
		showAnchorPreview: function (anchorEl) {
			if (this.anchorPreview.classList.contains('es-text-editor-anchor-preview-active')
			...
			clearTimeout(timer);
			timer = setTimeout(function () {
				if (self.anchorPreview && !self.anchorPreview.classList.contains('es-text-editor-anchor-preview-active')) {
					self.anchorPreview.classList.add('es-text-editor-anchor-preview-active');
				}
			}, 100);
		...
		createAnchorPreview: function () {
			...
			anchorPreview.id = 'es-text-editor-anchor-preview-' + this.id;
			anchorPreview.className = 'es-text-editor-anchor-preview';
		...
		anchorPreviewTemplate: function () {
			return '<div class="es-text-editor-toolbar-anchor-preview" id="es-text-editor-toolbar-anchor-preview">' +
				'    <i class="es-text-editor-toolbar-anchor-preview-inner"></i>' +
				'</div>';
		},
		...
		editorAnchorObserver: function (e) {
			...
			if (e.target && e.target.tagName.toLowerCase() === 'a') {
				...
				if (this.toolbar.classList.contains('es-text-editor-toolbar-active')) {
...
(function( $ ) {
	$.fn.JSNES_Editor = function( options ) {
		...
		return this.each(function( i, el ) {
			...
			if ($(el).data('es-text-editor-initialzed') == true)
				return;

			var $this = $(el);
			var mode = $(el).attr('data-mode') || options.mode;
			var bind =  $(el).attr('data-bind') || options.bind;
			var beforeEditContent;

			$this
				.addClass('es-text-editor')
				...
				case 'plain':
					$this
						...
						.on('keydown', function(e) {
							...
							if (e.keyCode == 27) {
								$this.text(beforeEditContent).blur();
							}
						})
						.on('keyup', function() {
							$this.find('br').remove();
					...
					break;
				case 'rich':
					if ($(el).data('medium-editor')){
						break;
					}
					var editor = new MediumEditor( el, {
						cleanPastedHTML: true,
						checkLinkFormat: true,
						anchorInputPlaceholder: 'http://',
						firstHeader: 'h1',
						secondHeader: 'h2',
						buttons: ['bold', 'italic', 'underline', 'superscript', 'subscript', 'anchor', 'header1', 'header2', 'unorderedlist', 'orderedlist', 'quote'],
						buttonLabels: {
							...
							'anchor': '<span class="fa fa-link"></span>',
							'header1': 'H1',
							'header2': 'H2',
							'unorderedlist': '<span class="fa fa-list-ul"></span>',
							'orderedlist': '<span class="fa fa-list-ol"></span>',
							'quote': '<span class="fa fa-quote-left"></span>'
						}
					});
					$this.data('medium-editor', editor);
					break;
			}
			...
			$this.blur(function( e ) {
				setTimeout(function() {
					if (this.innerHTML != beforeEditContent) {
						$this.trigger('change');
						beforeEditContent = this.innerHTML;
					}
				}.bind(this));
			});
		});
