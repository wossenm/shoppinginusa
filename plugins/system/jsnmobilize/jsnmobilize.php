...
class PlgSystemJSNMobilize extends JPlugin {
	...
	public function parseRule( &$router, &$uri ) {
		...
		if ( ! empty( $_SERVER[ 'HTTP_REFERER' ] ) ) {
			...
			if ( strpos( $_SERVER[ 'HTTP_REFERER' ], 'jsn_mobilize_preview=1' ) && ! isset( $_GET[ 'jsn_mobilize_preview' ] ) && empty( $_REQUEST[ 'format' ] ) ) {
	...
	public function buildRule( &$router, &$uri ) {
		$preview = ! empty( $_GET[ 'jsn_mobilize_preview' ] ) ? $_GET[ 'jsn_mobilize_preview' ] : '';
		$switch = ! empty( $_GET[ 'switch_to_desktop_ui' ] ) ? $_GET[ 'switch_to_desktop_ui' ] : '';
		if ( ! empty( $preview ) ) {
